...
serve(async (req) => {
  ...
  try {
    ...
    if (text.length > 2000) {
      return new Response(
        JSON.stringify({ error: 'Text too long. Maximum 2000 characters allowed for long-form content.' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    // Map language codes for VibeVoice
    ...
    try {
      // Call Hugging Face VibeVoice model
      const response = await fetch(
        'https://api-inference.huggingface.co/models/facebook/vibevoice-tts',
        {
          method: 'POST',
          headers: {
      ...
      console.error('VibeVoice primary failed, attempting Groq fallback:', hfError);
      return await fallbackWithGroq(text, language);
    }
