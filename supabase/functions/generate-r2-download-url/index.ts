...
serve(async (req)=>{
  ...
  try {
    ...
    // Request body
    const { fileUrl, expiresIn = 3600, bucketName } = await req.json();
    if (!fileUrl) {
    ...
    // Derive key from URL if full URL provided, otherwise treat as key
    const url = new URL(fileUrl, endpoint);
    // If fileUrl already includes bucket, pathname like /<bucket>/<key>
    // If not, assume fileUrl is a key and bucket provided separately
    let key;
    ...
    const signedUrl = await generatePresignedUrl(endpoint, CLOUDFLARE_ACCESS_KEY_ID, CLOUDFLARE_SECRET_ACCESS_KEY, bucket, key, 'GET', '', 0, expiresIn);
    return new Response(JSON.stringify({
      success: true,
